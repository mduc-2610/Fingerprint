...
def process_new_employee(
    employee_id,
    fingerprint_images_paths,
    segmentation_model,
    recognition_shape,
    segmentation_shape,
    ...
    processed_dir = os.path.join(
        script_dir, f"processed_fingerprints/employee_{employee_id}"
    )
    os.makedirs(processed_dir, exist_ok=True)

    fingerprints = []
    used_fingerprint_ids = []  # Danh sách lưu trữ các ID vân tay

    # Tạo đường dẫn đến file metadata
    metadata_file = os.path.join(processed_dir, "fingerprint_metadata.json")

    # Tải metadata nếu đã tồn tại
    ...
    for i, img_path in enumerate(fingerprint_images_paths):
        try:
            # Sử dụng fingerprint_id được cung cấp hoặc tạo mới
            if fingerprint_ids and i < len(fingerprint_ids):
                fingerprint_id = fingerprint_ids[i]
            else:
                fingerprint_id = str(uuid.uuid4())

            processed_img = preprocess_fingerprint(
                img_path, segmentation_model, recognition_shape, segmentation_shape
            )
            processed_path = os.path.join(processed_dir, f"fp_{fingerprint_id}.npy")
            np.save(processed_path, processed_img)

            # Lưu thông tin metadata cho vân tay
            metadata["fingerprints"][fingerprint_id] = {
            ...
            used_fingerprint_ids.append(fingerprint_id)
    ...
    if len(fingerprints) == 0:
        raise ValueError(
            f"No fingerprints were successfully processed for employee {employee_id}"
        )

    return np.array(fingerprints), used_fingerprint_ids
    ...
    for employee_id in os.listdir(dir_path):
        ...
        if os.path.isdir(employee_folder):
            ...
            for fingerprint_file in os.listdir(employee_folder):
                if fingerprint_file.endswith((".bmp", ".jpg", ".jpeg", ".png")):
                    employee_data[employee_id].append(
                        os.path.join(employee_folder, fingerprint_file)
                    )
...
def enroll_employees(
    ...
    recognition_shape,
    segmentation_shape,
    registered_fingerprint_ids=None,
):
    """Enroll employees and create embeddings database"""
    new_employees_data = {}
    all_fingerprint_embeddings = {}  # Lưu trữ embeddings theo fingerprint_id

    # Tạo dictionary ánh xạ employee_id -> danh sách fingerprint_ids được đăng ký
    fingerprint_id_map = {}
    if registered_fingerprint_ids:
        # Giả định rằng registered_fingerprint_ids là một danh sách các ID theo thứ tự
        # tương ứng với thứ tự trong employee_data_paths
        current_index = 0
        for employee_id, image_paths in employee_data_paths.items():
            fingerprint_id_map[employee_id] = []
            for _ in image_paths:
                if current_index < len(registered_fingerprint_ids):
                    fingerprint_id_map[employee_id].append(
                        registered_fingerprint_ids[current_index]
                    )
                    current_index += 1

    for employee_id, image_paths in employee_data_paths.items():
        try:
            # Truyền danh sách fingerprint_ids cho employee này nếu có
            employee_fingerprint_ids = fingerprint_id_map.get(employee_id)

            fingerprints, used_fingerprint_ids = process_new_employee(
                employee_id,
                image_paths,
                segmentation_model,
                recognition_shape,
                segmentation_shape,
                fingerprint_ids=employee_fingerprint_ids,
            )
            new_employees_data[employee_id] = (fingerprints, used_fingerprint_ids)
            print(
                f"Successfully enrolled employee {employee_id} with {len(fingerprints)} fingerprints"
            )
        except Exception as e:
    ...
    embeddings_db = {}
    fingerprint_embeddings_db = {}  # Database mới cho embeddings theo fingerprint_id

    for employee_id, (fingerprints, fingerprint_ids) in new_employees_data.items():
        # Tính toán embeddings cho mỗi vân tay
        employee_embeddings = embedding_model.predict(fingerprints, verbose=0)

        # Lưu trữ từng embedding riêng lẻ theo fingerprint_id
        for i, fp_id in enumerate(fingerprint_ids):
            fingerprint_embeddings_db[fp_id] = {
                "employee_id": employee_id,
                "embedding": employee_embeddings[i],
            }

        # Vẫn giữ lại cách lưu trữ cũ (trung bình) cho khả năng tương thích
        embeddings_db[employee_id] = np.mean(employee_embeddings, axis=0)
...
def update_model(
    ...
    if recognition_model is None or segmentation_model is None:
        print(
            "Failed to load models. Please check model paths and formats.",
            file=sys.stderr,
        )
        return False
    ...
    # Enroll employees
    try:
        embeddings_db, fingerprint_embeddings_db = enroll_employees(
            employee_data_paths,
            embedding_model,
            segmentation_model,
            recognition_shape,
            segmentation_shape,
            registered_fingerprint_ids,
        )
        return True
    except Exception as e:
...
def api_update_model():
    ...
    try:
        ...
        if success:
            return (
                jsonify({"status": "success", "message": "Model updated successfully"}),
                200,
            )
        else:
            return (
                jsonify({"status": "error", "message": "Failed to update model"}),
                500,
            )
...
def api_get_fingerprints():
    """API endpoint to get all registered fingerprints"""
    try:
