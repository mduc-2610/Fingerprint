...
function loadEmployeeStatistics() {
  ...
  fetch(
    `${BASE_URL}/api/employee/statistics?startDate=${startDate}&endDate=${endDate}`
  )
    .then((response) => response.json())
    .then((data) => {
      tableBody.innerHTML = "";
      ...
      data.forEach((stat) => {
        ...
        row.addEventListener("click", () =>
          loadEmployeeAccessLogs(stat.employeeId)
        );
        ...
        photoImg.src =
          stat.photoUrl ||
          "https://th.bing.com/th/id/OIP.hlxam_68Up5ge-CcOHvhLQHaHa?w=512&h=512&rs=1&pid=ImgDetMain";
...
function loadEmployeeAccessLogs(employeeId) {
  ...
  fetch(
    `${BASE_URL}/api/employee/${employeeId}/access-logs?startDate=${startDate}&endDate=${endDate}`
  )
    .then((response) => response.json())
    .then((data) => {
      tableBody.innerHTML = "";
...
function filterEmployeeAccessLogs() {
  ...
  const areaFilter = areaFilterElement.options[
    areaFilterElement.selectedIndex
  ].textContent
    .trim()
    .toLowerCase();
  const accessTypeFilter = accessTypeFilterElement.options[
    accessTypeFilterElement.selectedIndex
  ].textContent
    .trim()
    .toLowerCase();
  ...
  rows.forEach((row, index) => {
    ...
    const accessTypeMatch =
      accessTypeFilter === "all types" ||
      !accessTypeFilter ||
      accessType === accessTypeFilter;
    const areaMatch =
      areaFilter === "all areas" || !areaFilter || area === areaFilter;
...
function populateModelDropdown(elementId, models) {
  ...
  models.forEach((model) => {
    ...
    option.textContent = `${model.name} (Accuracy: ${(
      model.accuracy * 100
    ).toFixed(2)}%)`;
    dropdown.appendChild(option);
  });
...
function selectEmployee(employee) {
  ...
  document.getElementById("selectedEmployeePhoto").src =
    employee.photoUrl ||
    "https://th.bing.com/th/id/OIP.hlxam_68Up5ge-CcOHvhLQHaHa?w=512&h=512&rs=1&pid=ImgDetMain";
  document.getElementById(
    "selectedEmployeeFullName"
  ).textContent = `${employee.fullName}`;
  document.getElementById("selectedEmployeeId").textContent = employee.id;
  document.getElementById("selectedEmployeePhone").textContent =
    employee.phoneNumber || "-";
  document.getElementById("selectedEmployeeAddress").textContent =
    employee.address || "-";
  ...
  const selectedRow = document.querySelector(
    `#employeeTableBody tr[data-employee-id="${employee.id}"]`
  );
  if (selectedRow) {
...
function setupFilePreview() {
  ...
  document
    .getElementById("fingerprintFile")
  ...
  document
    .getElementById("fingerprintFileRecognize")
    .addEventListener("change", function (event) {
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      const fingerprintFile =
        document.getElementById("fingerprintFile").files[0];
      ...
      const fingerprintPosition = document.getElementById("fingerPosition")
        ? document.getElementById("fingerPosition").value
        : "RIGHT_INDEX";

      const segmentationModelId =
        document.getElementById("segmentationModel").value;
      const recognitionModelId = document.getElementById(
        "recognitionModelRegister"
      ).value;
      ...
      fetch(
        `${BASE_URL}/api/fingerprint/register/${selectedEmployee.id}/single`,
        {
          method: "POST",
          body: formData,
        }
...
function setupRecognizeForm() {
  ...
  document
    .getElementById("recognizeForm")
    .addEventListener("submit", function (event) {
      event.preventDefault();

      const fingerprintFile = document.getElementById(
        "fingerprintFileRecognize"
      ).files[0];
      ...
      const segmentationModelId = document.getElementById(
        "segmentationModelRecognize"
      ).value;
      const recognitionModelId =
        document.getElementById("recognitionModel").value;
...
function displayRegistrationResult(result) {
  ...
  resultContainer.innerHTML = `
        <div class="alert alert-success">
            ...
            <p>Successfully registered fingerprint for employee ${
              result.employeeId
            }.</p>
        </div>
        <div class="mt-3">
            ...
            <ul class="list-group mt-2">
                <li class="list-group-item">
                    Position: ${result.position}, Quality: ${(
    result.quality * 100
  ).toFixed(2)}%, 
...
function displayRecognitionResult(result) {
  ...
  const cardResult = document.getElementById("recognizeResult");
  ...
  if (result.matched) {
    resultContainer.innerHTML = `
            <div class="alert ${
              result.authorized ? "alert-success" : "alert-warning"
            }">
                <h5>${
                  result.authorized ? "Access Granted" : "Access Denied"
                }</h5>
                <p>Fingerprint matched with confidence: ${(
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <p><strong>Timestamp:</strong> ${new Date(
                      result.accessLog.timestamp
                    ).toLocaleString()}</p>
                    <p><strong>Area:</strong> ${
                      result.accessLog.area?.name || "General Access"
                    }</p>
                    <p><strong>Access Type:</strong> ${
                      result.accessLog.accessType
                    }</p>
    ...
    resultContainer.innerHTML = `
            <div class="alert alert-danger">
                <h5>Access Denied</h5>
                <p>No matching fingerprint found. Confidence: ${(
                  result.confidence * 100
                ).toFixed(2)}%</p>
            </div>
